...
export default class USDStage extends React.Component<
  USDStageProps,
      ...
      return (
        <li key={obj.name || index} className={listItemClass}>
          <div
            ...
            onClick={(e) => this._handleListClick(obj, e)}
            tabIndex={0}
          >
            {!isLeaf && (
              <span
                onClick={(e) => this._toggleExpand(obj, e)}
          ...
          {isOpen && !isLeaf && obj.children && (
            <ul className="nested-list">{this._renderList(obj.children)}</ul>
          )}
        </li>
      );
    });
    ...
    return (
      <div className="usdStageContainer" style={{ width: this.props.width }}>
        <div className="usdStageHeader">
          {"USD Stage"}
          <button className="nvidia-button" onClick={this._onReset}>
            Reset
          </button>
        </div>
