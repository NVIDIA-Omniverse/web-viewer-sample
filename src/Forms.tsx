    ...
    return (
      <div style={formContainerStyle}>
        ...
        <label>
          ...
          . It provides a user interface and functionality that supports Kit
          applications created from the <b>USD Viewer</b> Template in the{" "}
          <a
          ...
          .
          <br />
          If you are using this client to stream any other application you need
          to select the 2nd option below in order for the streamed application
          to become visible.
        </label>
        <br />
        <br />
 ...
 */
export class ServerURLsForm extends Component<
  ServerURLsProps,
  ServerURLsState
> {
  ...
  private _onBack(): void {
    let appServer = (document.getElementById("app-server") as HTMLInputElement)
      .value;
    let streamServer = (
      document.getElementById("stream-server") as HTMLInputElement
    ).value;
  ...
  async _onNext() {
    ...
    if (!(await this._validateEndpoint(`${streamServer}/streaming/stream`)))
      return;
  ...
  private _loadApplications = async (appServer: string) => {
    ...
    if (response.status === 200) {
      ...
      this.setState((prevState) => ({
        ...prevState,
        applications: Object.values(response.data),
      }));
    }
  };
              ...
              <div className="row align-items-center mb-3">
                ...
                <div className="col">
                  <input
                    ...
                    style={{ outline: "2px solid #76b900" }}
                  />
                </div>
              </div>
 ...
 */
export class ApplicationsForm extends Component<
  ApplicationsProps,
  ApplicationsState
> {
  ...
  async _onNext() {
    ...
    this.setState((prevState) => {
      if (prevState.versions.length === 0) {
        alert(
          `No versions were found from Application with id ${selectedAppId}`,
        );
        return;
      } else {
  ...
  handleSelectChange = (event: any) => {
    ...
    const selectedApplication = this.props.applications.find(
          ...
          >
            {this.props.applications.map((app) => (
              <option
                key={app.id}
                value={app.id}
                className="nvidia-dropdown-option"
              >
                {app.id}
              </option>
            ))}
...
export class VersionsForm extends Component<VersionsProps, VersionsState> {
  ...
  async _onNext() {
    ...
    this.setState((prevState) => {
      if (prevState.profiles.length === 0) {
        alert(
          `No profiles were found for Application version ${selectedVersion}`,
        );
        return;
      } else {
  ...
  private _loadProfiles = async (appId: string, version: string) => {
    const response = await getApplicationVersionProfiles(
      this.props.appServer,
      appId,
      version,
    );
    if (response.status === 200) {
      this.setState((prevState) => ({
        ...prevState,
        profiles: response.data.profiles.map((p) => p.id),
      }));
    }
  };
